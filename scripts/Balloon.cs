...
public partial class Balloon : Area3D
{
  ...
  private Godot.Tween IncreaseBalloonSizeTween(Vector3 finalScale, float duration)
  {
    ...
    tween.TweenProperty(this, property, finalScale, duration)
      .SetTrans(Godot.Tween.TransitionType.Sine)
      .SetEase(Godot.Tween.EaseType.Out);

    return tween;
  }

  private Godot.Tween StartFloatingUp()
  {
    string property = Node3D.PropertyName.Position.ToString();
    Godot.Vector3 finalPosition = new Godot.Vector3(this.Position.X, this.Position.Y * 2.5f, this.Position.Z);
    float randomDuration = GD.RandRange(1, 5);
    float duration = 2.5f * randomDuration;

    var tween = this.CreateTween();
    tween.TweenProperty(this, property, finalPosition, duration)
      .SetTrans(Godot.Tween.TransitionType.Sine)
      .SetEase(Godot.Tween.EaseType.Out);
